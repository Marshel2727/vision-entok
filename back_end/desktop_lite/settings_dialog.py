...
from ai.frame_sources import create_frame_source, redact_source
from ai.vigi_ptz import VigiPTZ, settings_from_source

from .model_registry import MODELS, model_by_id, resolve_model_path
from .runtime_paths import RuntimePaths
from .settings import DesktopSecrets, DesktopSettings
...
class SettingsDialog:
    def __init__(
        ...
        self.model_name = tk.StringVar(value=model_by_id(settings.model_id).name)
    ...
    def _build(self) -> None:
        ...
        model_combo = ttk.Combobox(
            model_box,
            state="readonly",
            values=[model.name for model in MODELS],
        ...
        for index, model in enumerate(MODELS):
            if model.id == self.initial_settings.model_id:
                ...
                break
        ttk.Label(model_box, text="Device").grid(row=1, column=0, sticky="w", pady=5)
        ttk.Combobox(
            model_box,
            state="readonly",
            values=["auto", "cpu", "0"],
        ...
        ttk.Label(model_box, text="Confidence").grid(row=2, column=0, sticky="w", pady=5)
        ttk.Spinbox(
            model_box,
            from_=0.05,
            to=0.95,
        ...
        ttk.Label(model_box, text="Ukuran inferensi").grid(row=3, column=0, sticky="w", pady=5)
        ttk.Combobox(
            model_box,
            state="readonly",
            values=[640, 768, 960],
    ...
    def _current(self) -> tuple[DesktopSettings, DesktopSecrets]:
        selected_model = next(
            (model for model in MODELS if model.name == self.model_name.get()),
            MODELS[0],
        )
        settings = replace(
