...
from dataset_preflight import validate_dataset_config
...
DEFAULT_DATA_YAML = PROJECT_DIR / "data" / "datasets" / "bebek.v3i.yolo26" / "data.yaml"
DEFAULT_OUTPUT_DIR = AI_DIR / "runs" / "train"
DEFAULT_MODEL_DIR = AI_DIR / "models"
DEFAULT_CONFIG = AI_DIR / "configs" / "train_yolo26m.yaml"
...
def parse_args():
    ...
    parser.add_argument(
        "--deploy-from",
        default=None,
        help="Salin weights dari folder run yang sudah diverifikasi ke ai/models, tanpa training ulang.",
    )
    parser.add_argument(
        "--preflight-only",
        action="store_true",
...
def main():
    ...
    model_name = get_value(args, config, "model", "yolo26m.pt")
    ...
    print(
        ...
        f'python ai/train.py --deploy-from "{results.save_dir}"'
    )
